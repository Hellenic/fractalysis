...
// Pre-calculations
vec3 scale_offset_1540259130 = offset * (scale - 1.0);
...
vec3 DodecahedronIFS(vec3 w)
{
    ...
    for (int i = 0; i < int(maxIterations); i++) {
        ...
        t = -w.x * c_0.x + w.y * c_0.y + w.z * c_0.z;
        if (t < 0.0) w += vec3(2.0, -2.0, -2.0) * t * c_0.xyz;

        t = w.x * c_0.z - w.y * c_0.x + w.z * c_0.y;
        if (t < 0.0) w += vec3(-2.0, 2.0, -2.0) * t * c_0.zxy;

        w *= fractalRotation2;
        w *= scale;
        w -= scale_offset_1540259130;
...
vec3 generateNormal(vec3 z, float d)
{
    ...
    float dx1 = DodecahedronIFS(z + vec3(e, 0, 0)).x;
    float dx2 = DodecahedronIFS(z - vec3(e, 0, 0)).x;

    float dy1 = DodecahedronIFS(z + vec3(0, e, 0)).x;
    float dy2 = DodecahedronIFS(z - vec3(0, e, 0)).x;

    float dz1 = DodecahedronIFS(z + vec3(0, 0, e)).x;
    float dz2 = DodecahedronIFS(z - vec3(0, 0, e)).x;
